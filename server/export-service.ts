  ...
  async exportQuotationToExcel(quotationId: string): Promise<Buffer> {
    try {
      ...
      const headerData = [
        ['QUOTATION'],
        [''],
        ['Date:', new Date(quotation.createdAt || new Date()).toLocaleDateString()],
        ['Quotation Number:', quotation.quotationNumber],
        ['Revision No:', quotation.revisionNumber || 'R1'],
        ['Customer Code:', quotation.customerCode],
        ['Country:', quotation.country],
        ['Price List:', quotation.priceListId || ''],
        [''],
        ['Items:']
      ];
  ...
  async exportSalesOrderToExcel(salesOrderId: string): Promise<Buffer> {
    try {
      ...
      const headerData = [
        ['SALES ORDER'],
        [''],
        ['Date:', new Date(salesOrder.createdAt || new Date()).toLocaleDateString()],
      ...
      itemsData.push(['', '', '', 'Shipping Fee:', salesOrder.shippingChargeUsd || '0']);
      ...
      switch (options.reportType) {
        ...
        case 'sales-orders':
          ...
          data = salesOrdersData.map(so => [
            new Date(so.createdAt || new Date()).toLocaleDateString(),
            so.salesOrderNumber,
            so.customerCode,
            so.country,
            so.status || 'Draft',
