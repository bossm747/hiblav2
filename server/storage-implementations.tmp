...
import {
  ...
  type InsertJobOrder,
  type JobOrderItem,
  type InsertJobOrderItem,
  type Warehouse,
  type InsertWarehouse,
  type ProductionReceipt,
  type InsertProductionReceipt,
  type Invoice,
  type InsertInvoice,
...
export interface IStorage {
  ...
  updateQuotationItem(id: string, item: Partial<InsertQuotationItem>): Promise<QuotationItem | undefined>;
  ...
  updateSalesOrderItem(id: string, item: Partial<InsertSalesOrderItem>): Promise<SalesOrderItem | undefined>;
  deleteSalesOrderItems(salesOrderId: string): Promise<boolean>;

  // Job Orders
  getJobOrders(): Promise<JobOrder[]>;
  getJobOrder(id: string): Promise<JobOrder | undefined>;
  getJobOrdersByCustomer(customerCode: string): Promise<JobOrder[]>;
  ...
  deleteJobOrder(id: string): Promise<boolean>;
  
  // Job Order Items
  getJobOrderItems(jobOrderId: string): Promise<JobOrderItem[]>;
  createJobOrderItem(item: InsertJobOrderItem): Promise<JobOrderItem>;
  updateJobOrderItem(id: string, item: Partial<InsertJobOrderItem>): Promise<JobOrderItem | undefined>;
  updateJobOrderItemShipment(itemId: string, shipmentNumber: number, quantity: string): Promise<JobOrderItem | undefined>;
  deleteJobOrderItems(jobOrderId: string): Promise<boolean>;
  ...
  createInventoryMovement(movement: InsertInventoryTransaction): Promise<any>;
  ...
  // Product Price Lookup (VLOOKUP functionality)
  getProductPrice(productId: string, priceListId: string): Promise<string | undefined>;
}
...
export class DatabaseStorage implements IStorage {
  ...
  // =====================================================
  // MANUFACTURING SYSTEM IMPLEMENTATIONS
  // =====================================================
  ...
  async updatePriceList(id: string, priceList: Partial<InsertPriceList>): Promise<PriceList | undefined> {
    const [updated] = await db.update(priceLists).set(priceList).where(eq(priceLists.id, id)).returning();
    return updated;
  }
  ...
  async getLatestQuotation(): Promise<Quotation | undefined> {
    const [quotation] = await db.select().from(quotations).orderBy(desc(quotations.quotationNumber)).limit(1);
    return quotation;
  }
  ...
  async updateQuotationItem(id: string, item: Partial<InsertQuotationItem>): Promise<QuotationItem | undefined> {
    const [updated] = await db.update(quotationItems).set(item).where(eq(quotationItems.id, id)).returning();
    return updated;
  }
  ...
  async updateSalesOrderItem(id: string, item: Partial<InsertSalesOrderItem>): Promise<SalesOrderItem | undefined> {
    const [updated] = await db.update(salesOrderItems).set(item).where(eq(salesOrderItems.id, id)).returning();
    return updated;
  }

  async deleteSalesOrderItems(salesOrderId: string): Promise<boolean> {
    const result = await db.delete(salesOrderItems).where(eq(salesOrderItems.salesOrderId, salesOrderId));
    return (result.rowCount ?? 0) > 0;
  }

  // Job Orders
  async getJobOrders(): Promise<JobOrder[]> {
    return await db.select().from(jobOrders).orderBy(desc(jobOrders.dateCreated));
  }
  ...
  async getJobOrdersByCustomer(customerCode: string): Promise<JobOrder[]> {
    return await db.select().from(jobOrders).where(eq(jobOrders.customerCode, customerCode)).orderBy(desc(jobOrders.dateCreated));
  }
  ...
  async updateJobOrder(id: string, jobOrder: Partial<InsertJobOrder>): Promise<JobOrder | undefined> {
    const [updated] = await db.update(jobOrders).set({ 
      ...jobOrder, 
      updatedAt: new Date() 
  ...
  async deleteJobOrder(id: string): Promise<boolean> {
  ...
  async updateJobOrderItem(id: string, item: Partial<InsertJobOrderItem>): Promise<JobOrderItem | undefined> {
    const [updated] = await db.update(jobOrderItems).set(item).where(eq(jobOrderItems.id, id)).returning();
    return updated;
  }

  async updateJobOrderItemShipment(itemId: string, shipmentNumber: number, quantity: string): Promise<JobOrderItem | undefined> {
    const shipmentField = `shipment${shipmentNumber}` as keyof typeof jobOrderItems;
    const [updated] = await db.update(jobOrderItems)
      .set({ [shipmentField]: quantity })
      .where(eq(jobOrderItems.id, itemId))
      .returning();
    return updated;
  }

  async deleteJobOrderItems(jobOrderId: string): Promise<boolean> {
    const result = await db.delete(jobOrderItems).where(eq(jobOrderItems.jobOrderId, jobOrderId));
    return (result.rowCount ?? 0) > 0;
  }
  ...
  async getInventoryMovements(filters: {
    ...
    if (filters.warehouseId) {
      query = query.where(eq(inventoryTransactions.warehouseId, filters.warehouseId)) as any;
    }
    if (filters.productId) {
      query = query.where(eq(inventoryTransactions.productId, filters.productId)) as any;
    }
    if (filters.movementType) {
      query = query.where(eq(inventoryTransactions.movementType, filters.movementType)) as any;
    }
    if (filters.dateFrom) {
      query = query.where(gte(inventoryTransactions.createdAt, new Date(filters.dateFrom))) as any;
    }
    if (filters.dateTo) {
      query = query.where(gte(new Date(filters.dateTo), inventoryTransactions.createdAt)) as any;
    }

    return await query.orderBy(desc(inventoryTransactions.createdAt));
  }

  async createInventoryMovement(movement: InsertInventoryTransaction): Promise<any> {
  ...
  async getManufacturingStats(): Promise<any> {
    const totalQuotations = await db.select({ count: sql`count(*)` }).from(quotations);
    const activeQuotations = await db.select({ count: sql`count(*)` }).from(quotations).where(eq(quotations.status, 'draft'));
    const totalSalesOrders = await db.select({ count: sql`count(*)` }).from(salesOrders);
    const confirmedSalesOrders = await db.select({ count: sql`count(*)` }).from(salesOrders).where(eq(salesOrders.isConfirmed, true));
    const totalJobOrders = await db.select({ count: sql`count(*)` }).from(jobOrders);

    return {
      totalQuotations: totalQuotations[0]?.count || 0,
      activeQuotations: activeQuotations[0]?.count || 0,
      totalSalesOrders: totalSalesOrders[0]?.count || 0,
      confirmedSalesOrders: confirmedSalesOrders[0]?.count || 0,
      totalJobOrders: totalJobOrders[0]?.count || 0,
    };
  }
  ...
  async getSummaryReport(filters: {
    ...
    if (filters.customerCode) {
      query = query.where(eq(salesOrders.customerCode, filters.customerCode)) as any;
    }
    if (filters.dateFrom) {
      query = query.where(gte(salesOrders.createdAt, new Date(filters.dateFrom))) as any;
    }
    if (filters.dateTo) {
      query = query.where(gte(new Date(filters.dateTo), salesOrders.createdAt)) as any;
    }

    const results = await query.orderBy(desc(salesOrders.createdAt));
    
    // Get items for each sales order if orderItems filter is provided
    if (filters.orderItems) {
      for (const order of results) {
        const items = await this.getSalesOrderItems(order.id);
        (order as any).items = items.filter(item => 
          item.productName.toLowerCase().includes(filters.orderItems!.toLowerCase())
        );
      }
      return results.filter((order: any) => order.items && order.items.length > 0);
    }
  ...
  async getPaymentReports(filters: {
    ...
    if (filters.customerId) {
      query = query.where(eq(customerPayments.customerId, filters.customerId)) as any;
    }
    if (filters.salesOrderId) {
      query = query.where(eq(customerPayments.salesOrderId, filters.salesOrderId)) as any;
    }
    if (filters.status) {
      query = query.where(eq(customerPayments.status, filters.status)) as any;
    }

    return await query.orderBy(desc(customerPayments.createdAt));
  }

  // Product Price Lookup (VLOOKUP functionality)
  async getProductPrice(productId: string, priceListId: string): Promise<string | undefined> {
    ...
    // Map price list names to product price fields
    switch (priceList.name.toLowerCase()) {
      case 'a':
      case 'price list a':
        return product.priceListA?.toString();
      case 'b':
      case 'price list b':
        return product.priceListB?.toString();
      case 'c':
      case 'price list c':
        return product.priceListC?.toString();
      case 'd':
      case 'price list d':
        return product.priceListD?.toString();
      default:
        return product.priceListA?.toString(); // Default to A
    }
  }
}
